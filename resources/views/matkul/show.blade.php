...
<head>
    ...
    <title>{{ $matkul->nama }} - EDOM UPS Tegal</title>
    ...
    <section style="padding:7rem 5% 3rem;position:relative;z-index:1;">
        <div style="max-width:900px;margin:0 auto;">
            ...
            <div style="display:flex;align-items:center;gap:0.5rem;font-size:0.8rem;color:rgba(255,255,255,0.35);margin-bottom:2rem;" class="fade-up">
                ...
                <a href="/search#matkul" style="color:rgba(255,255,255,0.35);text-decoration:none;transition:color 0.2s;" onmouseover="this.style.color='white'" onmouseout="this.style.color='rgba(255,255,255,0.35)'">Mata Kuliah</a>
                <i data-lucide="chevron-right" style="width:14px;height:14px;"></i>
                <span style="color:rgba(255,255,255,0.65);">{{ $matkul->nama }}</span>
            </div>
            ...
            " class="fade-up">
                ...
                <div style="
                    ...
                    flex-shrink:0;
                ">
                    <i data-lucide="book-open" style="width:36px;height:36px;color:var(--canary);"></i>
                </div>

                {{-- Info --}}
                <div style="flex:1;min-width:200px;">
                    <span class="section-label" style="margin-bottom:0.4rem;">Mata Kuliah</span>
                    ...
                    <p style="color:rgba(255,255,255,0.5);font-size:0.875rem;margin-bottom:1rem;">
                        {{ $matkul->jurusan->nama ?? '—' }} &middot; Semester {{ $matkul->semester }}
                    </p>
                    ...
                    <div style="display:flex;align-items:center;gap:0.5rem;flex-wrap:wrap;">
                        <div style="display:flex;align-items:center;gap:0.4rem;padding:0.35rem 0.85rem;background:rgba(255,246,78,0.1);border:1px solid rgba(255,246,78,0.2);border-radius:8px;">
                            <i data-lucide="hash" style="width:14px;height:14px;color:var(--canary);"></i>
                            <span style="font-size:0.8rem;color:var(--canary);font-weight:500;">{{ $matkul->kode }}</span>
                        </div>
                        <div style="display:flex;align-items:center;gap:0.4rem;padding:0.35rem 0.85rem;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;">
                            ...
                            <span style="font-size:0.8rem;color:rgba(255,255,255,0.6);">{{ $matkul->sks }} SKS</span>
                        </div>
                        <div style="display:flex;align-items:center;gap:0.4rem;padding:0.35rem 0.85rem;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;">
                            <i data-lucide="calendar" style="width:14px;height:14px;color:rgba(255,255,255,0.4);"></i>
                            <span style="font-size:0.8rem;color:rgba(255,255,255,0.6);">Semester {{ $matkul->semester }}</span>
                        </div>
                        <div style="display:flex;align-items:center;gap:0.4rem;padding:0.35rem 0.85rem;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:8px;">
                            <i data-lucide="landmark" style="width:14px;height:14px;color:rgba(255,255,255,0.4);"></i>
                            <span style="font-size:0.8rem;color:rgba(255,255,255,0.6);">{{ $matkul->jurusan->fakultas ?? '—' }}</span>
                        </div>
                    </div>
                </div>
                ...
                ">
                    ...
                    <div style="color:var(--canary);font-size:1rem;margin:0.3rem 0;letter-spacing:2px;">
                        @for($i = 1; $i <= 5; $i++){{ $i <= round($matkul->avg_rating) ? '★' : '☆' }}@endfor
                    </div>
                    <div style="font-size:0.75rem;color:rgba(255,255,255,0.35);">
                        {{ $matkul->total_review }} ulasan
                    </div>
                </div>
            ...
            " class="fade-up">
                ...
                <div style="display:flex;gap:2rem;flex-wrap:wrap;">
                    <div style="display:flex;flex-direction:column;align-items:center;gap:0.15rem;">
                        <span style="font-family:'Poppins',sans-serif;font-size:1.4rem;font-weight:800;color:var(--canary);">{{ $matkul->sks }}</span>
                        <span style="font-size:0.68rem;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:0.06em;">SKS</span>
                    </div>
                    <div style="display:flex;flex-direction:column;align-items:center;gap:0.15rem;">
                        <span style="font-family:'Poppins',sans-serif;font-size:1.4rem;font-weight:800;color:var(--canary);">{{ $matkul->semester }}</span>
                        <span style="font-size:0.68rem;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:0.06em;">Semester</span>
                    </div>
                    <div style="display:flex;flex-direction:column;align-items:center;gap:0.15rem;">
                        <span style="font-family:'Poppins',sans-serif;font-size:1.4rem;font-weight:800;color:var(--canary);">{{ $matkul->total_review }}</span>
                        <span style="font-size:0.68rem;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:0.06em;">Ulasan</span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    @for($star = 5; $star >= 1; $star--)
                        @php
                            $count = $matkul->reviews->where('rating', $star)->count();
                            $pct   = $matkul->reviews->count() > 0 ? round(($count / $matkul->reviews->count()) * 100) : 0;
                        @endphp
                ...
                {{-- Tags terpopuler --}}
                @if($matkul->reviews->count())
                <div style="flex:1;">
                    <div style="font-size:0.75rem;color:rgba(255,255,255,0.35);text-transform:uppercase;letter-spacing:0.08em;margin-bottom:0.75rem;">
                        Tag Terpopuler
                    </div>
                    @php
                        $allTags = $matkul->reviews->pluck('tags')->filter()->flatten()->countBy()->sortDesc()->take(8);
                    @endphp
                ...
                <a href="{{ route('review.create', ['matkul_id' => $matkul->id]) }}"
                   style="
            ...
            <div class="fade-up" id="ulasan">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1.5rem;">
                    <h2 style="font-family:'Poppins',sans-serif;font-weight:700;font-size:1.3rem;">
                        Ulasan Mahasiswa
                        <span style="color:rgba(255,255,255,0.3);font-size:1rem;font-weight:400;">({{ $matkul->total_review }})</span>
                    </h2>
                </div>

                {{-- 10 ulasan pertama --}}
                @forelse($matkul->reviews->take(10) as $index => $review)
                <div style="
                ...
                " onmouseover="this.style.borderColor='rgba(255,246,78,0.15)'" onmouseout="this.style.borderColor='rgba(255,255,255,0.07)'">
                    ...
                    {{-- Dosen badge (opsional) --}}
                    @if(isset($review->dosen))
                ...
                <div style="text-align:center;padding:4rem 0;">
                    ...
                    <p style="color:rgba(255,255,255,0.35);">Belum ada ulasan untuk mata kuliah ini.</p>
                    @auth
                    @if(Auth::user()->isMahasiswa())
                    <a href="{{ route('review.create', ['matkul_id' => $matkul->id]) }}" style="display:inline-flex;align-items:center;gap:0.5rem;margin-top:1rem;color:var(--canary);text-decoration:none;font-size:0.875rem;font-weight:600;">
                ...
                <div style="position:relative;margin-top:-1rem;">
                    ...
                    ">
                        ...
                        <h3 style="font-family:'Poppins',sans-serif;font-weight:700;font-size:1.1rem;margin-bottom:0.5rem;">
                            {{ $matkul->reviews->count() - 10 }} ulasan lainnya tersembunyi
                        </h3>
